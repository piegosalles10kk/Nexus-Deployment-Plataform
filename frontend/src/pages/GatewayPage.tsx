...
import { Activity, Plus, Trash2, Play, Pause, Search, RefreshCw, Layers, Globe, Zap, Settings } from 'lucide-react';
...
const GatewayPage: React.FC = () => {
  ...
  const [modalTab, setModalTab] = useState<'local' | 'public'>('local');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // If local mode, we let the backend handle path generation if empty
      // But we can also suggest one here
      await GatewayService.createRoute(formData);
      fetchRoutes();
      setShowModal(false);
      resetForm();
    } catch (error: any) {
  ...
  const resetForm = () => {
    setFormData({ name: '', routePath: '', targetUrl: '', checkPort: 0, isActive: true });
  };
  ...
  const selectPort = (port: number) => {
    setFormData({
      ...
      targetUrl: '', // Let backend handle it or set it manually
      checkPort: port,
      routePath: '' // Backend will generate /service/xxxx
    });
    setModalTab('local');
    setShowModal(true);
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800 backdrop-blur-xl">
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={() => { resetForm(); setModalTab('local'); setShowModal(true); }}
          ...
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8">
              <h2 className="text-xl font-bold text-zinc-100 mb-6 flex items-center gap-3">
                 {modalTab === 'local' ? <Zap className="text-amber-400" /> : <Globe className="text-emerald-400" />}
                 {modalTab === 'local' ? 'Configurar Serviço Local' : 'Criar Rota Externa'}
              </h2>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="flex gap-3 pt-6">
                  ...
                  <button 
                    type="submit"
                    className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20"
                  >
                    Salvar Rota
                  </button>
                </div>
